...
std::vector<std::vector<std::string>> items = {{"gold_bar", "art/gold_bar.bmp"}, {"gold_coin", "art/gold_coin.bmp"}};
...
SDL_Window* open_window(){
    if(SDL_Init(SDL_INIT_EVERYTHING))
        ...
        SDL_Window* window = SDL_CreateWindow("king of hearts",
...
text create_text(SDL_Renderer* ren, const char* textr, int size, SDL_Color rgb, TTF_Font* Sans, int x, int y){
...
void change_text(SDL_Renderer* ren, const char* textr, int size, TTF_Font* Sans, SDL_Color rgb, text * sprite_class){
    std::string textstr = textr;
    SDL_Surface* image = TTF_RenderText_Solid(Sans, textr, rgb);
    if(!image)
    {
...
sprite create_spite(SDL_Renderer* ren, const char* sprite_r, int x, int y, int w, int h, std::string name, std::string tag, int l){
...
void handle_buttons(SDL_Event e){
    for(int i=0; i < size(buttons); i++){
        ...
        if((e.type == SDL_MOUSEBUTTONDOWN) && (((buttons[i].x + buttons[i].w) > mouseX) && ((buttons[i].x - buttons[i].w) < mouseX)) && (((buttons[i].y + buttons[i].h) > mouseY) && ((buttons[i].y - buttons[i].h) < mouseY))){
            buttons[i].state = 1;
        }
        else if((((buttons[i].x + buttons[i].w) > mouseX) && ((buttons[i].x - buttons[i].w) < mouseX)) && (((buttons[i].y + buttons[i].h) > mouseY) && ((buttons[i].y - buttons[i].h) < mouseY))){
            buttons[i].state = 2;
        }
        else{
            buttons[i].state = 0;
...
void make_cards(SDL_Renderer *ren, int cards_to_make){
    ...
    for(int i=0; i < cards_to_make; i++){
        std::cout << "begin\n";
        cards_sprites.push_back(create_spite(ren,"art/card_back.bmp", (250 + i * 104)-(624*y), 250+y*86, 52, 86, "card"+i, "card", 5));
        ...
        if (sp_card == 1){cards_left -= 1; std::cout << "spc"; std::cout << cards_left << "\n";}
        std::cout << ">" << r << "\n";
...
void make_hearts(SDL_Renderer *ren, int health){
        for(int i=0; i < health; i++){
            hearts.push_back(create_spite(ren,"art/heart.bmp", 150 + i * (70 - (35 * floor(health/11))), 460, 52 - (27 * floor(health/11)), 52 - (27 * floor(health/11)), "heart"+i, "ui", 2));
        }
        rest_orders();
}
void load_level_(SDL_Renderer* ren, TTF_Font* Sans){
    ...
    // load ui
    spritesnc.push_back(create_spite(ren,"art/bottem_ui.bmp", 500, 400, 1000, 200, "bottem_tool_bar", "ui", 2));
    // dev tools
    // spritesnc.push_back(create_spite(ren,"art/heart.bmp", 150, 90, 50, 60, "damge_button", "ui", 2));
    // create_button(150,90,25,30, "ui");
    // spritesnc.push_back(create_spite(ren,"art/heart.bmp", 250, 90, 50, 60, "heal_button", "ui", 2));
    // create_button(250,90,25,30 , "ui");
    // spritesnc.push_back(create_spite(ren,"art/hand.bmp", 350, 90, 50, 60, "win_button", "ui", 2));
    // create_button(350,90,25,30, "ui");
    // spritesnc.push_back(create_spite(ren,"art/hand.bmp", 450, 90, 50, 60, "lose_button", "ui", 2));
    ...
    // player and hand
    player = create_spite(ren,"art/player.bmp", cards_sprites[0].sprite_rec.x, cards_sprites[0].sprite_rec.y, 50, 60, "player", "player", 2);
    hand = create_spite(ren,"art/hand.bmp", 0, 0, 50, 60, "player_hand", "ui", 1);
    texts.push_back(create_text(ren, (std::to_string(health_max) + "/" + std::to_string(health)).c_str(), 11, {0,0,0}, Sans, 650, 370));
    // Recache
    rest_orders();
}
...
void load_victory_ui(SDL_Renderer* ren, TTF_Font* Sans){
    spritesnc.push_back(create_spite(ren, "art/retry_button.bmp", 500, 400, 100, 80, "retry_button", "ui", 2));
    create_button(500, 400, 50, 40, "ui");
    if(win_lose_ui.name == "victory_sprite"){
        spritesnc.push_back(create_spite(ren, "art/next_button.bmp", 650, 400, 100, 80, "next_button", "ui", 2));
        ...
        for(int i=0; i < size(collected_items); i++){
            spritesnc.push_back(create_spite(ren, get_sprite_of_item(collected_items[i]).c_str(), 80, 500, 50, 80, "item"+i, "ui", 2));
            texts.push_back(create_text(ren, (std::to_string(collected_items_amount[i])+"x").c_str(), 12, {0,0,0}, Sans, 200, 500));
        }
    }
    rest_orders();
}
    ...
    if(win_lose_ui.name == "victory_sprite"){
        ...
        for(int i=2; i < size(spritesnc); i++){
            if(spritesnc[i].sprite_rec.y >= 200+((i-2)*100)){
                spritesnc[i].sprite_rec.y -= round((abs(spritesnc[i].sprite_rec.y-(200+((i-2)*100)))/10)+0.5);
                texts[i-1].text_rec.y -= round((abs(texts[i-1].text_rec.y-(200+((i-2)*100)))/10)+0.5);
            }
        }
    }
}   
...
void load_defeat_victory_ui(SDL_Renderer* ren, TTF_Font * Sans){
    if (won == 1){
        destory_sprite(&win_lose_ui);
        win_lose_ui = create_spite(ren, "art/victory.bmp", 500, 500, 400, 150, "victory_sprite", "ui", 1);
        load_level = false;
        won = -1;
    }
    if (won == -2){
        destory_sprite(&win_lose_ui);
        win_lose_ui = create_spite(ren, "art/defeat.bmp", 500, 500, 400, 150, "defeat_sprite", "ui", 1);
        load_level = false;
        won = -1;
    }
    if(win_lose_ui.sprite_rec.y > 100){
        ...
        if(win_lose_ui.sprite_rec.y <= 100 && (win_lose_ui.name == "defeat_sprite") ||  (win_lose_ui.name == "victory_sprite")){
            std::cout << "won:" << won << "\n";
            if(won!=3){
                erase_level();
...
void tick_cards(SDL_Renderer* ren){
    ...
    {   
        if(cards_sprites[i].tag == "destoryed_card"){
            if(cards_sprites_des_frames[i] > 20){
                if(random(1, round((cards_sprites_des_frames[i]/5) + 0.5)) == 1){
                    change_sprite_width_height(cards_sprites[i].sprite_rec.w+random(0, 5), cards_sprites[i].sprite_rec.h, &cards_sprites[i]);
                }
                else{
                    change_sprite_width_height(cards_sprites[i].sprite_rec.w-random(0, 5), cards_sprites[i].sprite_rec.h, &cards_sprites[i]);
                }
        ...
        if(cards_sprites[i].tag == "card" && ((player.sprite_rec.x + 208) > cards_sprites[i].sprite_rec.x) && ((player.sprite_rec.x - 208) < cards_sprites[i].sprite_rec.x) && ((player.sprite_rec.y + 172) > cards_sprites[i].sprite_rec.y) && ((player.sprite_rec.y - 172) < cards_sprites[i].sprite_rec.y) && !(cards_sprites[i].sprite_rec.x == player.sprite_rec.x && cards_sprites[i].sprite_rec.y == player.sprite_rec.y)){
            change_sprite(("art/card_front" + std::to_string(card_sprites_indexs[i]) +".bmp").c_str(), &cards_sprites[i], ren);
        }
        else{
        ...
        if((random_card_index != -1 && (((moved_this_frame&&(cards_sprites[i].sprite_rec.x == player.sprite_rec.x && cards_sprites[i].sprite_rec.y == player.sprite_rec.y)) && cards_sprites[i].tag == "card" ) && random_card_index != card_sprites_indexs[i])&&card_sprites_indexs[i]!=1 && card_sprites_indexs[i]!=0)){
            std::cout << "ouch";
            health -= 1;
        }
        if((cards_sprites[i].sprite_rec.x == player.sprite_rec.x && cards_sprites[i].sprite_rec.y == player.sprite_rec.y)&&moved_this_frame&&cards_sprites[i].tag == "card"){
        ...
        if(card_sprites_indexs[i] == random_card_index && ((cards_sprites[i].sprite_rec.x == player.sprite_rec.x && cards_sprites[i].sprite_rec.y == player.sprite_rec.y)&&moved_this_frame) && cards_sprites[i].tag == "card"){
            cards_sprites[i].tag = "destoryed_card";
            change_sprite(("art/card_front" + std::to_string(card_sprites_indexs[i]) +".bmp").c_str(), &cards_sprites[i], ren);
            cards_left -= 1;
        }
    ...
    if(moved_this_frame){
        int r = -1;
            ...
            if(random_card.name == "rand_card"){
                change_sprite(("art/card_front" + std::to_string(r) +".bmp").c_str(), &random_card, ren);
            }
        }
    }
}
int main(int argc, char** argv){
    ...
    while(keep_window_open){
            ...
            if(load_level){
                ...
                change_text(ren , (std::to_string(health_max) + "/" + std::to_string(health)).c_str(), 11, Sans, {0,0,0}, &texts[0]);
                render_ui(ren);
